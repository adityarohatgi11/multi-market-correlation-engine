...
class E2ETestSuite:
    ...
    def generate_test_report(self, passed: int, failed: int, warnings: int):
        ...
        for group, test_names in feature_groups.items():
            ...
            if group_total > 0:
                status_emoji = "✅" if group_passed == group_total else "⚠️" if group_passed > 0 else "❌"
        ...
        for result in self.test_results:
            status_emoji = {"PASS": "✅", "FAIL": "❌", "WARN": "⚠️", "INFO": "ℹ️"}.get(result["status"], "❓")
        ...
        if failed == 0:
            ...
            print(f"\n⚠️  MOSTLY OPERATIONAL with {failed} minor issues. System is usable.")
        else:
